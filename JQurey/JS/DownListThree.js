...
//一级数据抽离
DownList.prototype.DetachmentOne = function(){
    var jsonData = this.jsonData;
    for(let i = 0;i<jsonData.length;i++){
        for(let j = 0;j<jsonData.length;j++)
        if(jsonData[j].parentID == jsonData[i].ID){
        console.log("ID:"+jsonData[j].ID+"Name:"+jsonData[j].Name+"parentID:"+jsonData[j].parentID);
